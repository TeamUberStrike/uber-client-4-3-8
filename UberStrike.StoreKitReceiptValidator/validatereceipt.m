...
const char * GetReceipt(const char *receiptPath, const char *verNumber, BOOL debugMode)
{
    global_bundleVersion = [NSString stringWithUTF8String:verNumber];
    NSLog(@"Initialized. Target BundleVersion=%@", global_bundleVersion);
    
    // Validate the receipt
    BOOL isValid = validateReceiptAtPath([NSString stringWithUTF8String:receiptPath]);
    
    const char * returnData;
    
    if(isValid)
        returnData = ConvertReceiptToXml(dictionaryWithAppStoreReceipt([NSString stringWithUTF8String:receiptPath]),debugMode);
    else
        returnData = "<MASRECEIPT>Invalid</MASRECEIPT>";
    
    return returnData;
}

const char *ConvertReceiptToXml(NSDictionary *receiptDict, BOOL debugMode)
{
    NSMutableString *receiptMutableString;
	receiptMutableString = [[NSMutableString alloc] init];
    NSMutableString *dictValue;
    dictValue = [[NSMutableString alloc] init];
    
    [receiptMutableString setString:@"<MASRECEIPT>\r\n"];
    
    for (id key in receiptDict) 
    {        
        [dictValue setString:[NSString stringWithFormat:@"%@", [receiptDict objectForKey:key]]];
        [receiptMutableString appendFormat:@"<%@>%@</%@>\r\n", key, 
                              [[[dictValue stringByReplacingOccurrencesOfString:@"&" withString:@"&amp;"] 
                                           stringByReplacingOccurrencesOfString:@">" withString:@"&gt;"]
                                           stringByReplacingOccurrencesOfString:@"<" withString:@"&lt;"], key];
    }
    
    [receiptMutableString appendString:@"</MASRECEIPT>"];
    
    if(debugMode)
    {
        NSLog(@"%@", receiptMutableString);
    }
    
    const char * returnData = [receiptMutableString UTF8String];

    [receiptMutableString release];
    [dictValue release];
...
BOOL validateReceiptAtPath(NSString * path)
{
    ...
    // I removed grabbing the bundle info from the current bundle, instead just hardcode what we expect to be in the receipt
    NSString *mainBundleVersion = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleShortVersionString"];
    NSLog(@"MainBundleVersion from parent App=%@", mainBundleVersion);
    ...
    NSLog(@"Receipt BundleVersion=%@", [receipt objectForKey:kReceiptVersion]);
    // NSLog(@"Receipt BundleIdentifier = %@", [receipt objectForKey:kReceiptBundleIdentifier]);
    
	NSMutableData *hash = [NSMutableData dataWithLength:SHA_DIGEST_LENGTH];
	SHA1([input bytes], [input length], [hash mutableBytes]);

	if ([global_bundleIdentifier isEqualToString:[receipt objectForKey:kReceiptBundleIdentifier]] &&
		 [global_bundleVersion isEqualToString:[receipt objectForKey:kReceiptVersion]] &&
