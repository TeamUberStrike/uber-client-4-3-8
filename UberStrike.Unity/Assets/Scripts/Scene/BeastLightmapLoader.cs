...
/// 4. Restores original ambient intensity for maps where the "Fix lighting" commit changed it
///
...
public static class BeastLightmapLoader
{
    ...
    static readonly Dictionary<string, string> SceneLightmapFolders = new Dictionary<string, string>
    {
...
#if UNITY_EDITOR
        RestoreInEditor(scene);
#endif
    }

#if UNITY_EDITOR
    static void RestoreInEditor(Scene scene)
    {
        string lightmapDir = FindLightmapFolder(scene.name);
        if (lightmapDir == null)
            return;

        var files = Directory.GetFiles(lightmapDir, "LightmapFar-*.exr")
            .Select(f => f.Replace("\\", "/"))
            .OrderBy(f =>
            {
        ...
        LightmapData[] lightmapData = new LightmapData[files.Length];
        int loaded = 0;
        for (int i = 0; i < files.Length; i++)
        {
            Texture2D tex = AssetDatabase.LoadAssetAtPath<Texture2D>(files[i]);
        ...
        int framesLeft = AssignDelayFrames;
        ...
        ticker = () =>
        {
            ...
            // LevelSpaceship is the lobby — it has its own working lighting and
            // directional light (character shadow). Only load lightmaps + assign
            // renderers, don't disable lights or change ambient.
            // Save the lobby lightmaps so we can restore them when returning from a map
            // (LightmapSettings.lightmaps is global — map loading overwrites it).
            if (sceneName == "LevelSpaceship")
            {
                lobbyLightmaps = cachedLightmaps;
                Debug.Log($"[BeastLightmapLoader] LevelSpaceship: Lightmaps assigned ({lobbyLightmaps.Length} saved), skipping light adjustment (lobby)");
                return;
            }
    ...
    static void AdjustSceneLights(string sceneName)
    {
        ...
        foreach (var light in allLights)
        {
            ...
            // Skip lobby lights — lobby has its own working lighting
            string lightScene = light.gameObject.scene.name;
            if (lightScene == "LevelSpaceship") continue;
...
public class BeastLobbyLightmapGuard : MonoBehaviour
{
    ...
    static void RestoreLobbyLightmaps()
    {
        ...
        Debug.Log($"[BeastLightmapLoader] Restored lobby lightmaps ({BeastLightmapLoader.lobbyLightmaps.Length} textures) " +
                  $"and {restored} directional light(s)");
    }
}
