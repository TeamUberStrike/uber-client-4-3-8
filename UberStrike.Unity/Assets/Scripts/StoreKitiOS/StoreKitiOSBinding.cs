...
#if UNITY_IPHONE
public class StoreKitiOSBinding
{
    ...
    public static List<StoreKitiOSTransaction> getAllSavedTransactions()
    {
        if( Application.platform == RuntimePlatform.IPhonePlayer )
		{
			// Grab the transactions and parse them out
			var json = _storeKitGetAllSavedTransactions();
			return StoreKitiOSTransaction.transactionsFromJson( json );
		}
		
		return new List<StoreKitiOSTransaction>();
    }
