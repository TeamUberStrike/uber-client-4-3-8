...
{
#if UNITY_IPHONE
	private List<StoreKitiOSProduct> _products;
	...
	void OnGUI()
	{
		...
		{
			bool canMakePayments = StoreKitiOSBinding.canMakePayments();
		...
		{
			StoreKitiOSBinding.restoreCompletedTransactions();
		}
		...
		if( GUI.Button( new Rect( xPos, yPos += heightPlus, width, height ), "Validate Receipt" ) )
		{
			...
			List<StoreKitiOSTransaction> transactionList = StoreKitiOSBinding.getAllSavedTransactions();
			if( transactionList.Count > 0 )
				StoreKitiOSBinding.validateReceipt( transactionList[0].base64EncodedTransactionReceipt, true );
		}
		...
		if( _products != null && _products.Count > 0 )
		{
			if( GUI.Button( new Rect( xPos, yPos, width, height ), "Purchase Random Product" ) )
			{
				...
				StoreKitiOSBinding.purchaseProduct( product.productIdentifier, 1 );
			}
		}
		...
		if( GUI.Button( new Rect( xPos, yPos += heightPlus, width, height ), "Validate Subscription" ) )
		{
			...
			foreach( var t in transactionList )
			{
				...
				{
					StoreKitiOSBinding.validateAutoRenewableReceipt( t.base64EncodedTransactionReceipt, "YOUR_SECRET_FROM_ITC", true );
					break;
				}
			}
		}
		...
		if( GUI.Button( new Rect( xPos, yPos += heightPlus, width, height ), "Get Saved Transactions" ) )
		{
			...
			foreach( StoreKitiOSTransaction transaction in transactionList )
