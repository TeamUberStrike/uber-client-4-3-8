...
public class SplineInterpolator : MonoBehaviour
{
	...
	internal class SplineNode
	{
        ...
        {
            Point = p; Rot = q; Time = t; EaseIO = io;
        }
    ...
    {
        get { return mState == "Stopped" ? true : false; }
    }
	...
	public void StartInterpolation(GameObject target, OnEndCallback endCallback, bool bRotations, eWrapMode mode)
	{
	...
	private void Update()
	{
		...
		if (mState != "Stopped")
		{
			...
			// Smooth the param
			float sparam = MathUtils.Ease(Mathf.Clamp(param, 0, 0.999999f), mNodes[mCurrentIdx].EaseIO.x, mNodes[mCurrentIdx].EaseIO.y);

            //print("param = " + param + " sparam = " + sparam + " New position = " + GetHermiteInternal(mCurrentIdx, sparam));

            _currentPosition = GetHermiteInternal(mCurrentIdx, param);
            _currentRotation = GetSquad(mCurrentIdx, sparam);

            if (mTarget)
            {
                mTarget.transform.position = _currentPosition;
                if (mRotations) mTarget.transform.rotation = _currentRotation;
            }
		}
	}
