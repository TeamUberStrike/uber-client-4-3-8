...
public class Sprite2D : IAnimatable2D
{
    ...
    public void Draw(float offsetX = 0.0f, float offsetY = 0.0f)
    {
        ...
        if (isDrawn)
        {
            ...
            GUI.DrawTexture(spriteRect, _texture);
    ...
    public Vector2 Size
    {
        get
        {
            _size.x = Scale.x * _texture.width;
            _size.y = Scale.y * _texture.height;
            return _size;
        }
    }
    ...
    private void UpdateRect()
    {
        if (_texture)
        {
            ...
            _rect.y = Position.y;
            _rect.width = _texture.width * Scale.x;
            _rect.height = _texture.height * Scale.y;
        }
        else
        {
            throw new CmuneException("UpdateRect failed because Texture is NULL");
        }
    }
